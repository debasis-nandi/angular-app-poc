"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var forms_1 = require('@angular/forms');
var common_1 = require('@angular/common');
var event_modal_component_1 = require('./event-modal.component');
var modal_module_1 = require('../modals/modal.module');
var mydaterangepicker_1 = require('mydaterangepicker');
var mydatepicker_1 = require('mydatepicker');
var EventModalModule = (function () {
    function EventModalModule() {
    }
    EventModalModule = __decorate([
        core_1.NgModule({
            imports: [
                common_1.CommonModule,
                forms_1.FormsModule,
                modal_module_1.ModalModule,
                mydaterangepicker_1.MyDateRangePickerModule,
                mydatepicker_1.MyDatePickerModule
            ],
            declarations: [
                event_modal_component_1.EventModalComponent
            ],
            exports: [
                event_modal_component_1.EventModalComponent
            ]
        }), 
        __metadata('design:paramtypes', [])
    ], EventModalModule);
    return EventModalModule;
}());
exports.EventModalModule = EventModalModule;
//# sourceMappingURL=event-modal.module.js.map